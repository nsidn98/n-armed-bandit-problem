...
def learn(tasks_num,arm_value_mean,arm_value_sd,arms_num,arm_reward_sd,
	...
	for task_index in range(0,tasks_num):
		...
		if with_random_walks:
			...
			arm_mean_values=np.random.normal(arm_value_mean,arm_value_sd,arms_num)
		estimates=[0.0]*arms_num
		tries=[0]*arms_num
		for playIndex in range(0,plays_num):
			arm_index=explorer_function(params[0],arms_num,estimates)
			reward=np.random.normal(arm_mean_values[arm_index],arm_reward_sd)  #arm_reward_sd=std deviation
			tries[arm_index] += 1
			if step_size==0:
				estimates[arm_index]=(estimates[arm_index]+1.0/float(tries[arm_index])*(reward-estimates[arm_index]))
			else:
				estimates[arm_index]=(estimates[arm_index]+step_size*(reward-estimates[arm_index]))
			task_result.append(reward)
			if with_random_walks:
				for arm_index in range(0,arms_num):
					step=np.random.choice((-random_walk_step,random_walk_step))	
					arm_mean_values[arm_index] +=step
		results.append(task_result)			
		update_progress(float(task_index)/float(tasks_num-1)*100)
	return results
...
plot_results(learn(2000,0.0,1.0,10,1.0,1000,0.1,True ,0.1,epsilon_greedy,0.1),"Learning with Epsilon Greedy")			
plot_results(learn(2000,0.0,1.0,10,1.0,1000,0.01,True ,0.1,epsilon_greedy,0.1),"Learning with Epsilon Greedy")						
